...
<script>
import { getAllCityData } from "@/api/province";
export default {
    props: {
        title: {
            type: String,
    ...
    created() {
        this._getAllCityData(this.$route.query.name);
    },
    mounted() {
        // 动态引入省份地图
        this.loadMap();
    ...
    methods: {
        async _getAllCityData(province) {
            console.log(province);
            const { data } = await getAllCityData(province);
            this.data = data;
        ...
        async initChart() {
            ...
            const option = {
                ...
                tooltip: {
                    show: true,
                    //params为echarts>>series中data的遍历，但是data的值为空
                    formatter: params => {
                        // console.log("radar=>params:" + JSON.stringify(params));
                        console.log(params);
            ...
            console.log("abd" + JSON.stringify(option.series[0]));
        }
    }
};
</script>
